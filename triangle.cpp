/*
Отрисовка треугольника взята из предыдущего урока
по позиционированию графических элементов в графической
сцене, а вот перерисовка в слоте, который будет обрабатывать
 сигнал от таймера, а также инициализация первичного
 поворота объекта уже является новым куском кода.

Поворот объекта задается в градусах переменной angle
 и устанавливается функцией setRotation(),
которая была унаследована от QGraphicsItem.
Также для отслеживания состояния кнопок клавиатуры
используется функция из WinAPI, а именно GetAsyncKeyState()
, которая по коду кнопки определяет состояние этой самой
 кнопки. При каждом сигнале от объекта класса QTimer
 происходит проверка нажатых клавиш и в зависимости от их
 состояния изменяется положение треугольника. */
#include "triangle.h"
...
//всё рисование должно быть ограничено внутри этого прямоугольника.
QRectF Triangle::boundingRect() const
{
    return QRectF(-40,-50,80,100);   /// Ограничиваем область, в которой лежит треугольник
}
//А этот метод отвечает за перерисовку
...
void Triangle::slotGameTimer()
{
       ...
       GetAsyncKeyState(VK_DOWN))
    {
        ...
        if(countForSteps == 4){
            steps = 2;
 /*Планирует перерисовку области этого элемента покрываемой прямоугольником rect.
             *  Вы можете вызывать эту функцию когда необходимо перерисовать элемент, например,
             *  когда у него изменяется вид или размер.
              Эта функция не приводит к незамедлительной отрисовке; Вместо этого она планирует запрос отрисовки,
             который будет обработан QGraphicsView после того как управление вернётся в цикл событий.
             Элемент будет перерисован только в том случае, если он виден в любом связанном виде.

            Побочным эффектом перерисовки элемента будет то, что элементы, которые перекрывают область rect,
            могут быть также перерисованы.
* */
